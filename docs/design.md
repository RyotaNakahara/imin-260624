...
3. 各候補日について「出席可 / 出席不可」（必要なら「未定」）を選択できる
...
| 削除 | ホストが候補日を削除した場合、既存回答の該当列は無効化（または削除） |

### 3.3 ゲスト回答（Response）
| 項目 | 仕様 |
|------|------|
| 表示名 | 必須、最大 50 文字 |
| 各候補への回答 | `available` / `unavailable`（MVP は 2 値。将来 `maybe` 追加可） |
...
├── eventId         : string (FK)
├── type            : "date" | "datetime"
├── startAt         : datetime  // date の場合は 00:00 UTC で保存
└── sortOrder       : int
...
| DB | **SQLite + Prisma** | セットアップ軽量、MVP に十分 |
| スタイル | **Tailwind CSS** | 高速 UI 実装 |
| ID 生成 | **nanoid** | 短く推測困難なトークン |
| 日時 | **date-fns** + `Intl` | タイムゾーン表示（MVP は JST 固定でも可） |
...
| 2 | 「未定」オプション | MVP では省略 |
| 3 | タイムゾーン | 日本時間（JST）固定表示 |
| 4 | ホストの候補日編集 | 追加・削除可。既存回答がある候補の削除は確認ダイアログ |
| 5 | データベース本番 | MVP は SQLite。トラフィック増時に Postgres へ移行 |

---

## 14. 次のアクション

1. **未決事項の確認**（上記 §13。異論なければデフォルトで進行）
2. **Agent Team セットアップ** — `.cursor/agents/` と rules の作成
3. **Phase 0 開始** — プロジェクト初期化・Prisma スキーマ
4. **並列実装** — Phase 2a / 2b を frontend エージェントで分担
